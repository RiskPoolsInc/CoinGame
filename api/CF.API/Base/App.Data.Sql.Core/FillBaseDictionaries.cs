...
";
 ...
 ";
    ...
    #region insert Setting property

    public static string InsertSettingProperty(SettingProperty entity) {
        var valueNumber = entity.ValueNumber?.ToString() ?? "null";
        var valueNumberDouble = entity.ValueNumberDouble?.ToString() ?? "null";
        var valueText = string.IsNullOrWhiteSpace(entity.ValueText) ? "null" : $"'{entity.ValueText}'";

        var query =
            @$"
insert into ""SettingProperties"" (""Id"", ""CreatedOn"", ""UpdatedOn"", ""TypeId"", ""ValueNumber"", ""ValueNumberDouble"", 
                                   ""ValueText"", ""ValueTypeId"")
            values ('{entity.Id:D}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 
            {entity.TypeId},{valueNumber},{valueNumberDouble},{valueText},{entity.ValueTypeId});
";
        return query;
    }
...
public static class MigrationBuilderExtension {
    ...
    public static void InsertTranTaxSettingProperties(this MigrationBuilder migrationBuilder) {
        //Insert Wallet for taz
        migrationBuilder.InsertSettingProperty(new SettingProperty {
            Id = Guid.NewGuid(),
            TypeId = 2,
            CreatedOn = DateTime.UtcNow,
            UpdatedOn = DateTime.UtcNow,
            ValueTypeId = (int)SettingPropertyValueTypes.Text,
            ValueText = "Ux98e1829d64d686c62d165639f8de481aacc885bd"
        });

        //Insert Wallet Tax walue
        migrationBuilder.InsertSettingProperty(new SettingProperty {
            Id = Guid.NewGuid(),
            TypeId = 3,
            CreatedOn = DateTime.UtcNow,
            UpdatedOn = DateTime.UtcNow,
            ValueTypeId = (int)SettingPropertyValueTypes.Int,
            ValueNumber = 1000
        });
    }
    ...
    public static void InsertSettingProperty(this MigrationBuilder migrationBuilder, SettingProperty entity) {
        migrationBuilder.Sql(FillBaseDictionaries.InsertSettingProperty(entity));
    }
}