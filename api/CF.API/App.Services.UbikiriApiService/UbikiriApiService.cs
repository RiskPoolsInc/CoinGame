...
using App.Core.ViewModels.ExternalSystems.Ubikiri;
...
public class UbikiriApiService {
    ...
    public async Task<UbikiriSocialProperty[]> GetUserSocialProfiles() {
        var path = _ubikiriSettings.Url;
        path += "/users/current/properties/social";
        var properties = await Get<UbikiriSocialProperty[]>(path);
        return properties as UbikiriSocialProperty[];
    }
